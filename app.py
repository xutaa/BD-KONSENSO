...
@app.route('/armazem/editar', methods=['POST'])
@login_required
def editar_armazem():
    ...
    try:
        ...
        if not conn:
            ...
            return redirect(url_for('lista_armazens'))
        cursor = conn.cursor()
        cursor.execute("{CALL dbo.AtualizarArmazem (?, ?, ?)}", (id, localizacao, capacidade))
        conn.commit()
        flash('✅ Armazém atualizado com sucesso!', 'success')
    except Exception as e:
        error_msg = str(e).split(']')[-1] if ']' in str(e) else str(e)
...
@app.route('/cargo/editar', methods=['POST'])
@login_required
def editar_cargo():
    ...
    try:
        ...
        if not conn:
            ...
            return redirect(url_for('lista_cargos'))
        cursor = conn.cursor()
        cursor.execute("{CALL dbo.AtualizarCargo (?, ?, ?)}", (id, nome, descricao))
        conn.commit()
